...
/**
 * HeaderMofa - MOFA(외교부) 스타일 헤더 레이아웃
 *
 * 특징:
 * - 3단 구조: 유틸리티 바 → 로고 영역 → 메인 네비게이션
 * - 스크롤 시 네비게이션 고정 (sticky)
 * - 드롭다운 서브메뉴 지원
 * - 모바일 햄버거 메뉴
 */
...
interface HeaderMofaProps {
  className?: string
}

export default function HeaderMofa({ className }: HeaderMofaProps) {
  ...
  const navItems: NavigationItem[] = [
    ...
    },
    {
      ...
      href: '/mauritania',
      children: [
        { label: locale === 'ko' ? '지리' : locale === 'en' ? 'Geopolitics' : locale === 'fr' ? 'Géopolitique' : 'الجيوسياسة', href: '/mauritania/geography' },
            ...
            <ul className={cn('flex items-center', isRTL && 'flex-row-reverse')}>
              <li className="flex items-center">
                <a
                  ...
                  className="flex items-center gap-1 sm:gap-2 px-2 sm:px-4 text-[11px] sm:text-[13px] text-gray-600 hover:text-gray-900 whitespace-nowrap"
                >
                  <span>🌐</span>
                  <span>MAECME</span>
                </a>
              ...
              <li className="flex items-center before:content-[''] before:w-px before:h-3 before:bg-gray-300">
                <a
                  ...
                  className="flex items-center gap-1 sm:gap-2 px-2 sm:px-4 text-[11px] sm:text-[13px] text-gray-600 hover:text-gray-900 whitespace-nowrap"
                >
                  <svg className="w-4 h-3 rounded-sm flex-shrink-0" viewBox="0 0 3000 2000">
                  ...
                  <span className="hidden sm:inline">{locale === 'ko' ? '대통령실' : locale === 'en' ? 'Presidential Office' : locale === 'fr' ? 'Présidence' : 'الرئاسة'}</span>
                  <span className="sm:hidden">{locale === 'ar' ? 'الرئاسة' : locale === 'fr' ? 'Prés.' : locale === 'en' ? 'Pres.' : '대통령'}</span>
                </a>
              </li>
            </ul>
